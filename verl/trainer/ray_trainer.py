...
class RayPPOTrainer:
    ...
    def _validate(self) -> Dict[str, Any]:
        ...
        gen_config = GenerationConfig(
            ...
            search_url = self.config.data.retriever_url,
            topk = self.config.data.retriever_topk,
        )
        ...
        print(f"Total number of batches in self.val_dataloader: {len(self.val_dataloader)}")
        for batch_dict in self.val_dataloader:
            ...
            test_batch1 = test_batch.repeat(repeat_times=repeat_times, interleave=True)
            test_gen_batch1 = test_gen_batch.repeat(repeat_times=repeat_times, interleave=True)
            test_gen_batch1.meta_info = self.config.worker.rollout.val_override_config
            test_gen_batch1.meta_info["min_pixels"] = self.config.data.min_pixels
            test_gen_batch1.meta_info["max_pixels"] = self.config.data.max_pixels
            test_gen_batch1.meta_info["video_fps"] = self.config.data.video_fps
            test_gen_batch1, pad_size = pad_dataproto_to_divisor(test_gen_batch1, self.actor_rollout_ref_wg.world_size)
            
            # breakpoint()
            with timer("step", timing_raw):
                    first_input_ids = test_gen_batch1.batch['input_ids'][:, -gen_config.max_start_length:].clone()
                    with timer("gen", timing_raw):
                        generation_manager.timing_raw = timing_raw
                        # breakpoint()
                        test_output_gen_batch = generation_manager.run_llm_loop(
                            gen_batch=test_gen_batch1,
                            initial_input_ids=first_input_ids,
                            gen_batch1=test_gen_batch
                        )
            test_output_gen_batch = unpad_dataproto(test_output_gen_batch, pad_size=pad_size * repeat_times)
            
            
            # tool1
            test_batch = test_batch1.union(test_output_gen_batch)
            ...
            accuracy_values = reward_metrics['accuracy']
            # 指定要保存的 JSON 文件名
            # filename = "/home/chenhui/EasyR1/accuracy.json"
        ...
        max_dim = max(tensor.size(1) for tensor in reward_tensor_lst)  # 找出第 1 维的最大大小
        padded_tensors = []
        for tensor in reward_tensor_lst:
            padding_size = max_dim - tensor.size(1)  # 计算需要填充的大小
            if padding_size > 0:
                # 在第 1 维上填充零
        ...
        self.val_reward_score = torch.cat(padded_tensors, dim=0).sum(-1).mean().item()
    ...
    def _make_batch_data(self, metrics: Dict[str, Any]) -> DataProto:
        ...
        while True:
            ...
            new_batch1 = new_batch.repeat(repeat_times=self.config.worker.rollout.n, interleave=True)
            # new_batch1.meta_info=meta_info
            # breakpoint()
            new_batch1.meta_info=meta_info.copy()
            ...
            # pop those keys for generation
            gen_batch1 = new_batch1.pop(
                batch_keys=["input_ids", "attention_mask", "position_ids"],
                non_tensor_batch_keys=["raw_prompt_ids", "multi_modal_data"],
                meta_info_keys=["min_pixels", "max_pixels", "video_fps"],
            )    
 
            first_input_ids = gen_batch1.batch['input_ids'][:, -gen_config.max_start_length:].clone()
            gen_batch_output = generation_manager.run_llm_loop(
                                gen_batch=gen_batch1,
                                initial_input_ids=first_input_ids,
                                gen_batch1=gen_batch
                            )
            
            # grpo
            # generate a batch
            # gen_batch_output = self.actor_rollout_ref_wg.generate_sequences1(gen_batch1)
            ...
            new_batch = new_batch1.union(gen_batch_output)
            ...
            with timer("step", timing_raw):
                ...
                with timer("adv", timing_raw):
                    if "token_level_scores" not in batch.batch:
                        ...
                        with open("/home/chenhui/EasyR1/loss_grpo.txt", "a") as f:
                            f.write(f"reward:{reward_metrics}\n")
