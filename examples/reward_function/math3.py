...
def format_reward(response: str) -> float:
    pattern = re.compile(
        r"^"
        r"<think>.*?</think>"  # 开头的think块（必须有一个）
        r"(?:(?:<search>.*?</search>|<information>.*?</information>|<think>.*?</think>)*)"  # 中间任意顺序的块
        r"<answer>.*?</answer>"
        r"$",
        re.DOTALL
    )
...
def compute_score(reward_inputs: List[Dict[str, Any]], format_weight: float = 0.4) -> List[Dict[str, float]]:
